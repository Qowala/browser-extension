const app = new Vue({
    ...
    tracked: function () {
      return this.config && this.config.blacklist &&
             (this.config.blacklist.includes(this.activeURL) ||
              (this.config.blacklist.includes(this.activeURL.substring(4)) &&
               this.activeURL.startsWith('www.')))
    },
    topSites: function () {
  ...
  methods: {
    track: function () {
      ...
      if (this.hostname.startsWith('www.') && (this.hostname.match(/\./g) || []).length === 2) {
        this.hostname = this.hostname.substring(4)
      }
