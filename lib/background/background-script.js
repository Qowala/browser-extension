...
window.setInterval(() => {
  chrome.storage.local.get({
    ...
    init(result).then(wasInitialized => {
      if (!wasInitialized) {
...
async function init (result) {
  if (result.config.firstTime) {
    ...
    for (const url of defaultConfig.websites) {
      sites.push(Website.fromUrl(url, `${url}/favicon.ico`))
    }
    result.config.websites = result.config.websites.concat(await Promise.all(sites))
  }
